...
class ELO {
  ...
  static Future<List<StudyRoute>> getStudyRoutes() async {
    final String url =
        'https://elo.windesheim.nl/services/Studyroutemobile.asmx/LoadStudyroutes?start=0&length=100&filter=0&search=';
    Response<dynamic> response = await makeRequest(url);

    final rawRoutes = (response.data['STUDYROUTES'] as List<dynamic>)
        .map((e) => e as Map<String, dynamic>)
        .toList();
    final List<StudyRoute> routes =
        rawRoutes.map((raw) => StudyRoute.fromJson(raw)).toList();
    return routes;
  }

  static Future<List<StudyContent>> getStudyContent(
      int studyRouteId, int? parentId) async {
    final String url =
        "https://elo.windesheim.nl/services/studyroutemobile.asmx/LoadStudyrouteContent?studyrouteid=${studyRouteId}&parentid=${parentId ?? -1}&start=0&length=100";
    Response<dynamic> response = await makeRequest(url);
    final rawContent = (response.data['STUDYROUTE_CONTENT'] as List<dynamic>)
        .map((e) => e as Map<String, dynamic>)
        .toList();
    final List<StudyContent> content =
        rawContent.map((raw) => StudyContent.fromJson(raw)).toList();
    return content;
  }

  static Future<HandinDetails> getHandinDetails(int resourceId) async {
    final String url =
        "https://elo.windesheim.nl/services/Studyroutemobile.asmx/LoadUserHandinDetails?studyRouteResourceId=${resourceId}";
    Response<dynamic> response = await makeRequest(url);
    final rawContent = ((response.data['STUDYROUTE_USER_HANDINDETAILS']
  ...
  static Future<bool> downloadFile(String url, String path, CancelToken token,
    ...
    try {
      var response = await Dio().download(url, path,
          ...
          cancelToken: token,
          options:
              Options(headers: {"Cookie": "N%40TCookie=${prefs.eloCookie}"}));
